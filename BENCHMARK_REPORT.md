...
[![Benchmark Status](https://img.shields.io/badge/Benchmark-20%2F50%20Completed-blue.svg)](#batch-progress)
...
[![Tests Passing](https://img.shields.io/badge/Tests-315%20passing-success.svg)](#test-suite-health)
...
|:---|:---|:---|
| **Repositories Tested** | **20 / 50** | **40.0%** |
| 🟢 **Full Success (`FULL_SUCCESS`)** | **10** | **50.0%** |
| 🟡 **Partial Success (`PARTIAL_SUCCESS`)** | **8** | **40.0%** |
| 🔵 **Correctly Unsupported (`CORRECTLY_UNSUPPORTED`)** | **2** | **10.0%** |
| 🔴 **Engine Defects (`INCORRECT_FAILURE`)** | **0** | **0.0%** |
| **Deterministic Engine Reliability** | **18 / 18 runnable** | **100.0%** |

> **Key Takeaway**: Across 20 distinct real-world repositories encompassing monorepos, microframeworks, distributed web architectures, and polyglot libraries, RunRepo achieved a **0% unhandled engine failure rate**. Every runnable project successfully reached dependency installation, service orchestration, and background application startup.
...
3. 🔵 **`CORRECTLY_UNSUPPORTED`**: Project requires a system compiler/runtime absent on the host OS (e.g. Rust/Cargo for native extensions, incompatible pinned Node major versions). RunRepo cleanly detected the requirement and safely stopped without crashing.
...
2. **Zero-Pollution Virtual Environments**:
   * Automatic creation and inspection of Python `.venv` environments using ultra-fast `uv`.
   * Automatic repair of broken virtual environments via `uv venv --clear`.
3. **Automated Backing Service Provisioning**:
   * Scans root and subfolders for `docker-compose.yml` (excluding tests/integration harnesses).
   * Automatic fallback container spawning with dynamic collision-free port allocation (PostgreSQL, Redis).
4. **Dynamic Port Discovery & Verification**:
...
* **Unit & Integration Tests**: 315 tests passing (`uv run pytest`) in ~77 seconds.
* **Regression Test Suite**: 8 dedicated end-to-end regression tests in `tests/test_batch_regressions.py`.
...
# Run a specific repository by ID (e.g. FastAPI = 16)
uv run python tests/real_world/runner.py --id 16
```
